...
def main():
    ...
    dictLib = defaultdict(int); dictCon = defaultdict(int); q = float(sys.argv[3])
    ...
    nLib = sum(dictLib.values()) + (float(q) * nVocab)
    nCon = sum(dictCon.values()) + (float(q) * nVocab)
    testNBClassifier(probLib, probCon, nLib, nCon, dictLib, dictCon, q)

# function to test Naive Bayes classifier
def testNBClassifier(probLib, probCon, nLib, nCon, dictLib, dictCon, q):
    ...
    for everyFName in fileNames:
        ...
        for everyWord in readFile:
            if everyWord.lower().strip() in dictLib or everyWord.lower().strip() in dictCon:
                nkLib = dictLib[everyWord.lower().strip()] + q
                nkCon = dictCon[everyWord.lower().strip()] + q
